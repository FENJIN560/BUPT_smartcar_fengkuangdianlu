...
IFX_INTERRUPT(uart2_rx_isr, 0, UART2_RX_INT_PRIO)
{
    ...
    uart_rxbuf[rx_write] = rx_temp_byte;//将接受到的字节数据存入缓冲区中
    rx_write = (rx_write + 1) & (UART_BUF_SIZE - 1);

    uart_query_byte(UART_2,&rx_temp_byte);
    fifo_write_buffer(&uart_data_fifo, &rx_temp_byte, 1);


//    Joystick_Process_Byte(uart2_rx);
//    uart_query_byte(UART_2,&uart2_rx);
